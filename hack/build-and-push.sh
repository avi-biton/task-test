...
function is_official_repo() {
    ...
    grep -Eq '^(quay\.io/)?(redhat-appstudio-tekton-catalog|konflux-ci/tekton-catalog|avi-test/catalog)(/.*)?$' <<< "$1"
}

QUAY_NAMESPACE="avi-test/catalog"
BUILD_TAG="12345"
...
build_push_tasks() {
    ...
    find task/*/* -maxdepth 0 -type d | awk -F '/' '{ print $0, $2, $3 }' | while read -r task_dir task_name task_version
