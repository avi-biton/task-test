...
DATA_BUNDLE_REPO="${DATA_BUNDLE_REPO:-quay.io/avi_test/user-ns2/release-catalog/data-acceptable-bundles}"
mapfile -t BUNDLES < <(cat "$@")

pr_number=$(gh search prs --repo avi-biton/task-test --merged "${REVISION}" --json number --jq '.[].number')

# changed files in a PR
mapfile -t changed_files < <(gh pr view "https://github.com/avi-biton/task-test/pull/${pr_number}" --json files --jq '.files.[].path')
